...
const casesPath = process.env.CALMCSS_VARIANT_CASES ?? 'test/variant_cases.json'
const cases = JSON.parse(await readFile(casesPath, 'utf8'))
const maxDiffs = Number.parseInt(process.env.CALMCSS_PARITY_DIFFS ?? '20', 10)

await spawnFile('zig', ['build'], { stdio: 'inherit' })

const dir = await mkdtemp(join(tmpdir(), 'calmcss-variant-parity-'))
let pass = 0
let fail = 0
const diffs = []
...
try {
  for (const testCase of cases) {
    if (!Array.isArray(testCase.candidates)) {
      throw new Error(`variant parity case "${testCase.name}" must define a candidates array`)
    }
    if (typeof testCase.css !== 'string') {
      throw new Error(`variant parity case "${testCase.name}" must define css`)
    }
    ...
    const document = testCase.htmlBeforeCss
      ? `${html}<style>${testCase.css}</style>`
      : `<style>${testCase.css}</style>${html}`
...
console.log(`CalmCSS/Tailwind custom variant parity: ${pass}/${total} (${pct.toFixed(2)}%)`)
